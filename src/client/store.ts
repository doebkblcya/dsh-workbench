...
/** Workbench panel width contract (the single right column hosting Files/Git). */
export const MIN_WORKSPACE_PANEL_PX = 240
export const MAX_WORKSPACE_PANEL_PX = 600
export const DEFAULT_WORKSPACE_PANEL_PX = 320
...
/**
 * Workbench panel clamp (runs first): reserve chat's floor plus the preview
 * region (min + chrome) when open, so the panel never grows into their space;
 * floor at the panel minimum so a narrow container cannot squeeze it to nothing.
 */
...
/**
 * Preview clamp (runs last): reserve chat's floor plus the already-clamped
 * panel width plus the region chrome. The ordered pair guarantees
 * chat = available - panel - preview >= 360.
 */
...
export interface LayoutState {
  ...
  root: string
  /** Requested panel width (persisted; clamped on render). */
  explorerWidth: number
  /** Requested preview width (persisted; clamped on render). */
  previewWidth: number
  /** Panel collapsed (width 0, kept mounted). */
...
export interface LayoutStore extends StateHandle<LayoutState> {
  /** Effective panel width after the ordered clamp. */
...
/** Git column state (branches / graph / commit / switch / sync). */
export interface GitState {
  root: string
  /** Local branch list (null: not a repo / not loaded). */
  branches: BranchesView | null
  /** Inline commit graph. */
  graph: GraphView | null
  branchesLoading: boolean
  graphLoading: boolean
  /** Whether the graph load has settled (null graph then means "not a repo"). */
  graphLoaded: boolean
  commitMessage: string
  committing: boolean
  syncing: boolean
  switching: boolean
  /** Last switch/create rejection (mapped to copy at render). */
  switchError: PanelError | null
  /** Last commit rejection. */
...
export interface GitStore extends StateHandle<GitState> {
  setRoot: (root: string) => void
...
/**
 * Create the git store. `onGitChanged` fires after a successful mutation
 * (commit/switch/create/push/pull) so the caller can refresh the scm status,
 * the preview diffs, and the explorer tree.
 */
export function createGitStore(api: PanelApi, onGitChanged: () => void): GitStore {
  const handle = createState<GitState>({
    root: '',
    branches: null,
    graph: null,
  ...
  const store: GitStore = Object.assign(handle, {
    setRoot(root: string) {
      handle.update((prev) => {
        if (prev.root === root) return prev
        return {
    ...
    async loadBranches() {
      ...
      handle.update((prev) => ({ ...prev, branchesLoading: true }))
      const result = await api.branches(root)
      handle.update((prev) => {
        if (prev.root !== root || seq !== branchesSeq) return prev
        return { ...prev, branches: result.ok ? result.value : null, branchesLoading: false }
      })
    },
    async loadGraph(limit?: number) {
      ...
      if (root === '') return
      const seq = ++graphSeq
      handle.update((prev) => ({ ...prev, graphLoading: true }))
      const result = await api.graph(root, limit)
      handle.update((prev) => {
        if (prev.root !== root || seq !== graphSeq) return prev
        // Once the load settles, a null graph means "not a repository" (the
        // host returns OK(null) for non-repos), never "still loading" — the
        // auto-retry in GraphPanel keys off `graphLoaded` to avoid looping.
        return { ...prev, graph: result.ok ? result.value : null, graphLoading: false, graphLoaded: true }
      })
    },
    setCommitMessage(message: string) {
      handle.update((prev) => (prev.commitMessage === message ? prev : { ...prev, commitMessage: message, commitError: null }))
    },
    async switchBranch(branch: string) {
      const root = handle.getSnapshot().root
      if (root === '') return false
      handle.update((prev) => ({ ...prev, switching: true, switchError: null }))
      const result = await api.switchBranch(root, branch)
      if (result.ok) {
        handle.update((prev) => ({ ...prev, switching: false }))
        onGitChanged()
        void store.loadBranches()
        void store.loadGraph()
        return true
      }
      handle.update((prev) => ({ ...prev, switching: false, switchError: result.error }))
      return false
    },
    async createBranch(name: string) {
      ...
      if (root === '') return false
      handle.update((prev) => ({ ...prev, switching: true, switchError: null }))
      const result = await api.createBranch(root, name)
      if (result.ok) {
        handle.update((prev) => ({ ...prev, switching: false }))
        onGitChanged()
        void store.loadBranches()
    ...
    async commit() {
      const state = handle.getSnapshot()
      const root = state.root
      if (root === '' || state.commitMessage.trim() === '') return false
      handle.update((prev) => ({ ...prev, committing: true, commitError: null }))
      const result = await api.commit(root, state.commitMessage)
      if (result.ok) {
        handle.update((prev) => ({ ...prev, committing: false, commitMessage: '' }))
        onGitChanged()
        void store.loadBranches()
        void store.loadGraph()
        return true
...
/** Convenience bundle: the five stores wired to one api. */
...
export function createPanelStores(api: PanelApi): PanelStoresWithFlush {
  ...
  // The git store is created after the others so its change hook can reach them.
  let git: GitStore
  const onGitChanged = (): void => {
    const root = scm.getSnapshot().root
    void scm.refresh()
    void preview.handleGitChange(root)
    void explorer.handleFsChange()
  }
  git = createGitStore(api, onGitChanged)
  const flushNow = (): void => {
  ...
  const stores: PanelStoresWithFlush = { layout, explorer, scm, preview, git, flushNow }
  return stores
}
